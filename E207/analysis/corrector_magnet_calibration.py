...
data = np.loadtxt("corrector_magnet_calibration_C1_z.txt", skiprows=1)
angles = data[:, 0]
...
plt.plot(
    ...
    func(angles, *popt),
    "r-",
    label=r"fit: $z(\alpha)=(%.3f \pm %.3f)\, \frac{\text{mm}}{\text{mrad}} \cdot \alpha + (%.2f \pm %.2f)\, \text{mm}$"
...
plt.plot(
    angles,
    func(angles, L_theo/1000, popt[1]),
    "g-",
    label="expected slope (true length)",
)
plt.xlabel(r"$\alpha_z$ in mrad")
plt.ylabel(r"$z$ in mm")
plt.grid()
